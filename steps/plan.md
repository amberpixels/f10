...
Context: load per `context.md` — guardrails and conventions come from
...
1. **Design a solid, conventional solution.** No hacky shortcuts, no reinventing the wheel.
   Reuse existing abstractions and honour the project's quality bar — its CLAUDE.md
   conventions and the guardrails declared in project.md.
...
3. **Produce a staged plan:** ordered steps, the critical files each touches, any
   ...
   user's call, each with the default the plan proceeds on (see `gaps.md`). No real forks →
   `## Gaps — none`.
...
4. **Save the plan — this IS the deliverable, not optional polish.** Write it with the `Write`
   ...
   untracked per the project's visibility (stealth → `.git/info/exclude`, see `context.md`).
   ...
   **Out-of-tree storage** (context resolution found the instructions at `~/.f10/<project>/` —
   see `context.md → Storage`): everything above applies with the storage root swapped in —
...
5. **Confirm and hand off.** Verify the file exists, present the plan in chat, and report its
   ...
   now** via a batched `AskUserQuestion` (see `gaps.md`) — an offer the user can decline; the plan
