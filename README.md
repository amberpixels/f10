...
- **Demo** - the optional answer to *what was built*, for the moment before you merge a PR whose
  ...
  is offered one: a single questionnaire pre-filled from what was detected, written to
  `.f10/instructions/demo.md`, and the run carries on. Evidence is budgeted by claim, not by
  count, before/after is captured only where something visible already existed, and the report
  carries rows only for artifacts that were actually written. Finds no bugs and reaches no
  verdict - that is `review` and `judge`.
